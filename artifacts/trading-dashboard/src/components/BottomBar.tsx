  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 p-3 bg-background/90 backdrop-blur-md border-t shadow-[0_-10px_40px_rgba(0,0,0,0.05)] dark:shadow-[0_-10px_40px_rgba(0,0,0,0.3)] z-50 flex gap-2">
      <Button 
        ...
        variant="success"
        disabled={state === 'running' || startEngine.isPending}
      ...
      <Button 
        className="flex-1 h-14 text-base font-bold shadow-sm"
        variant={state === 'running' ? 'warning' : 'outline'}
        disabled={state !== 'running' || pauseEngine.isPending}
      ...
      <Button 
        ...
        variant="destructive"
        disabled={(state !== 'running' && state !== 'paused') || stopEngine.isPending}
